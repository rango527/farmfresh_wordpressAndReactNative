		...
		if(!empty($wcfm_delivery_orders_array)) {
			foreach( $wcfm_delivery_orders_array as $wcfm_delivery_order_single ) {
				...
				if( $wcfm_delivery_order_single->vendor_id ) {
					$store = '<span class="wcfm_vendor_store">' . apply_filters( 'wcfm_vendors_store_name_data', $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_by_vendor( $wcfm_delivery_order_single->vendor_id ), $wcfm_delivery_order_single->vendor_id ) . '</span>';
					if( apply_filters( 'wcfm_is_allow_store_location_to_delivery_boys', true ) ) {
						...
						if( $store_user->get_address_string() )
							$store .= '<div class="wcfm_store_location"><i class="wcfmfa fa-map-marker" aria-hidden="true"></i>&nbsp;<a href="https://maps.google.com/?q=' . rawurlencode( $store_user->get_address_string() ) . '&z=16" target="_blank">' . $store_user->get_address_string() . '</a></div>';
					}
