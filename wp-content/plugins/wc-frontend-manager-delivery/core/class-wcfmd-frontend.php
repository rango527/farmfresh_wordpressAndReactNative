...
class WCFMd_Frontend {
	...
	public function __construct() {
		...
		if( !apply_filters( 'wcfm_is_pref_delivery', true ) ) return;
  ...
  function wcfmd_delivery_menus( $menus ) {
  	...
  	if( apply_filters( 'wcfm_is_allow_delivery', true ) ) {
			...
			$menus = array_slice($menus, 0, 3, true) +
											array( 'wcfm-delivery-boys' => array(  'label'      => __( 'Delivery Boys', 'wc-frontend-manager-delivery'),
  ...
  function wcfmd_delivery_settings( $wcfm_options ) {
		...
		if( !apply_filters( 'wcfm_is_allow_delivery', true ) ) return;
			  ...
			  <div class="wcfm_clearfix"></div>
				<?php
					...
					$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfmd_settings_fields_general', array(
																																																"wcfm_group_auto_assign" => array('label' => __('Group auto-assignment on Registration', 'wc-frontend-manager-delivery'), 'name' => 'wcfm_group_auto_assign', 'type' => 'checkbox', 'class' => 'wcfm-checkbox wcfm_ele '. $group_auto_assign, 'label_class' => 'wcfm_title checkbox_title wcfm_ele '. $group_auto_assign, 'value' => 'yes', 'dfvalue' => $wcfm_group_auto_assign ),
	...
	function wcfmd_capability_settings_delivery( $wcfm_capability_options ) {
		...
		<?php
			$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_capability_settings_fields_delivery', array(  
																																	 "delivery" => array('label' => __('Delivery', 'wc-frontend-manager-delivery') , 'name' => 'wcfm_capability_options[delivery]', 'type' => 'checkboxoffon', 'class' => 'wcfm-checkbox wcfm_ele', 'value' => 'yes', 'label_class' => 'wcfm_title checkbox_title', 'dfvalue' => $delivery),
																										) ) );
