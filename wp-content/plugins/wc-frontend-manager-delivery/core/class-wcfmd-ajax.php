		...
		if( $delivery_id ) {
			...
			if( !empty( $delivery_details ) ) {
				foreach( $delivery_details as $delivery_detail ) {
					...
					$wcfm_messages = sprintf( __( 'Order <b>%s</b> item <b>%s</b> delivered by <b>%s</b>.', 'wc-frontend-manager-delivery' ), '#<span class="wcfm_dashboard_item_title">' . $delivery_detail->order_id . '</span>', get_the_title( $delivery_detail->product_id ), $wcfm_delivery_boy_user->first_name . ' ' . $wcfm_delivery_boy_user->last_name );
